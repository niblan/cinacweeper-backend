...
# generate board with indexes and without mines.
    def generate_board(self, height: int, width: int) -> list[list]:
        '''
        Create a board.
    ...
    def set_mines(self, height: int, width: int, num_mines: int, step: tuple) -> list:
        '''
    ...
    def get_info_board(self, height: int, width: int, mines: list) -> list[list]:
        '''
    ...
    def flag(self, board: list[list], step: tuple) -> None:
        '''
        Set or delete flag.
    ...
    def end_game(self, board: list[list], info_board: list[list]):
        height = len(board)
        width = len(board[0])
    ...
    def check_ceil(self, board: list[list], info_board: list[list], step: tuple) -> None:
        '''
    ...
    def get_step(self, height, width) -> tuple:
        ...
        try:
            ...
            if row < height and col < width:
                ...
                print(f'Enter integers! {height}, {width}, {row}, {col}')
                self.get_step(height, width)
        except Exception as e:
            print(f'Enter integers! {e}')
            self.get_step(height, width)
    ...
    def main(self):
        ...
        mines = self.set_mines(height, width, num_mines, (4, 4)) # there is no mines at (4, 4)
