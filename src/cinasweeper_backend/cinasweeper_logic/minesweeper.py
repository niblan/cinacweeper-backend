...
def check_ceil(board: list[list], info_board: list[list], step: tuple) -> None:
    '''
...
def main():
    ...
    board = generate_board(height, width) # create board
    ...
    mines = set_mines(height, width, num_mines, (4, 4)) # there is no mines at (4, 4)
    info_board = get_info_board(height, width, mines)
    ...
    while True:
        ...
        if action:
            check_ceil(board, info_board, get_step(height, width))
        else:
            flag(board, get_step(height, width))
