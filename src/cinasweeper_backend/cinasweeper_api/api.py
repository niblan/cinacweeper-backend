...
from ..cinasweeper_logic import Game, GameMode, Move, User  # {перелік класів}
...
def create_game(jwt: str, gamemode: GameMode):
    ...
    return database.create_game(owner=user, gamemode=gamemode)
...
class Game:
    ...
    def from_logic(cls, game):
        return Game(
            game.id,
            game.owner and manager.get_user(game.owner),
            game.started,
            game.started_time,
            game.game_mode,
        )
